...
public class Project {
    ...
    public boolean canProjectActiveFunc(){
        if(managerOfTheProject!=null&& employees.stream().filter(e -> e instanceof Analyst).count() >= minNumberOfAnalyst && employees.stream().filter(e -> e instanceof Designer).count() >= minNumberOfDesigner && employees.stream().filter(e -> e instanceof Programmer).count() >= minNumberOfProgrammer){
                return true;
        }
        return false;
    }
