		...
		addWorkerButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (nameField.getText().isEmpty() || surnameField.getText().isEmpty()  || respBox.getSelectedItem() == "Seçiniz..." ||  timeBox.getSelectedItem() == "Seçiniz..." ) {
					...
					try {
						boolean isadded=ysoft.addNewEmployee(nameField.getText(), surnameField.getText(),respBox.getSelectedItem().toString(), timeBox.getSelectedItem().toString());
					//eklenemediyse yazdirilmali
						...
						JOptionPane.showMessageDialog(null, "Veritabaniyla Baglanti Hatasi!!", "Hata", JOptionPane.ERROR_MESSAGE);
